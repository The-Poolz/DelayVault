...
contract DelayView is DelayManageable {
    ...
    {
        _delay = getLimits(_token, _amount, DelayLimit[_token].MinDelays);
    }

    function GetCliffTime(address _token, uint256 _amount)
        public
        view
        returns (uint256 _cliffTime)
    {
        _cliffTime = getLimits(_token, _amount, DelayLimit[_token].CliffTimes);
    }
    ...
    ) internal view returns (uint256 _delay) {
        if (
